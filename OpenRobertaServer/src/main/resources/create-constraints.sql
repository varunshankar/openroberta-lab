...
create unique index groupIdx on USER(GROUP_ID);
...
alter table GROUP add foreign key (OWNER_ID) references USER(ID) ON DELETE CASCADE;
commit;

create unique index groupOwnerNameIdx on GROUP(OWNER_ID, NAME);
commit;

alter table ACCESS_RIGHT_HISTORY add foreign key (GROUP_ID) references GROUP(ID) ON DELETE CASCADE;
commit;
...
alter table GROUP               		alter column ID generated by default as identity (start with 93);
alter table ACCESS_RIGHT_HISTORY        alter column ID generated by default as identity (start with 93);
commit;
